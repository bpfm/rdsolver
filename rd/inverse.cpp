lapack_int mat_inv(double *A, unsigned n, double X, double Y, int ID){
        int ipiv[n+1];
        lapack_int ret;
        ...
        if(ret !=0){
                std::cout << "B WARNING: MATRIX CANNOT BE INVERTED\t" << ret << "\t(0 = done, <0 = illegal arguement, >0 = singular) at " << X << "\t" << Y << "\tPoint =\t" << ID << std::endl;
                for(int i=0;i<n*n;++i){
                        std::cout << A[i] << "\t";
