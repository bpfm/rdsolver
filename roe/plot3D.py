...
x4,y4,z4,rho4 = data[9*n_points:10*n_points,0]-0.50*dx, data[9*n_points:10*n_points,1]-0.50*dx, data[9*n_points:10*n_points,2]-0.50*dx, data[9*n_points:10*n_points,3]
...
for j in range(5):
        ...
        #for i in range(n_points):
                #print(x0[i])
        #        x_bin = int(x4[i]*n_side/side_size)
        #        y_bin = int(y4[i]*n_side/side_size)
        #        z_bin = int(z4[i]*n_side/side_size)
        #        density_grid1[x_bin,y_bin,z_bin] = rho4[i]

        density_collapse_z = np.sum(density_grid,1)/float(n_side)
        #density_collapse_z1 = np.sum(density_grid1,1)/float(n_side)

        density_collapse_yz = np.sum(density_collapse_z,1)/float(n_side)
        #density_collapse_yz1 = np.sum(density_collapse_z1,1)/float(n_side)

        plt.figure(100+j)

        plt.pcolor(x_range,y_range,density_collapse_z,cmap=plt.cm.rainbow)
        ...
        #plt.plot(x_range,density_collapse_yz1)
